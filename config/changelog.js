...
const RELEASES = [
  ...
  },
  {
    ...
    entries: [
      ...
      'New: Nothing model-backed starts without saying what it costs. A dialog before every run lists what it actually does step by step with the price of each, the requests, tokens and minutes it expects, whether those numbers were measured or are a guess, and that nothing is written while it runs; levers to make it cheaper update the numbers and the button as you tick them',
      'New: While a run works, the page shows what it has spent: tokens so far against the estimate, split into the question, the answer and the model thinking to itself, the request in flight against its budget, and the last requests one by one with what each cost and how it ended — including a request that thought for tokens and answered nothing',
